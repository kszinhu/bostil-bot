...
use serenity::{
  all::{
    AutoArchiveDuration, ButtonStyle, ChannelId, ChannelType, CommandDataOption, CommandOptionType,
    ComponentInteraction, InputTextStyle, User,
  },
  async_trait,
  builder::{
    ...
    let mut embed_message = match SETUP_EMBED.send_message(&ctx, &thread_channel).await {
      ...
      Err(_) => {
        ...
        return Ok(CommandResponse::String(
          t!("commands.poll.setup.response.error", "thread_id" => thread_channel.id).to_string(),
        ));
      }
    };
            ...
            CreateSelectMenu::new(
              ...
              CreateSelectMenuKind::String {
                options: vec![
                  ...
                  CreateSelectMenuOption::new("Múltipla escolha", "multiple_choice")
