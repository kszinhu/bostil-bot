...
pub async fn join_channel(channel: &ChannelId, ctx: &Context, user_id: &UserId) -> () {
	...
	let user = match User::get_by_id(connection, user_id) {
		Some(user) => user,
		None => {
			info!(
			...
			let client_user = match ctx.http.get_user(*user_id).await {
				Ok(user) => user,
				Err(why) => {
			...
			match User::new(*user_id, &client_user.name.as_str(), false).save(connection) {
				Ok(user) => user,
				Err(e) => {
	...
	let message = CACHE.with(|cache| {
		...
		if let Some((counter, last_update, _)) = cache.get_mut(user_id) {
			if now - *last_update < 5 {
				...
				*counter += 1;
				...
				if user.username == "scaliza" {
					if members.len() == 1 {
						return t!("interactions.join_channel.scaliza.empty_channel", user_id => user_id)
							.to_string()
							.into();
					} else if members.len() >= 3 {
						return t!("interactions.join_channel.scaliza.many_users", user_id => user_id)
							.to_string()
							.into();
					}
			...
			if user.username == "scaliza" {
				return t!("interactions.join_channel.scaliza.0", user_id => user_id)
					.to_string()
					.into();
			}

			return t!("interactions.join_channel.0", user_id => user_id)
				.to_string()
				.into();
		}
	});
