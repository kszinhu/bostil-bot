...
impl PollModel {
    pub fn from(poll: &Poll, votes: Vec<Vote>, user_id: &UserId) -> PollModel {
        PollModel {
            votes,
            id: poll.id,
            kind: poll.kind,
    ...
    pub fn from_yaml(yaml: &Yaml) -> PollModel {
        PollModel {
            ...
            created_by: UserId(yaml["created_by"].as_i64().unwrap().try_into().unwrap()),
        }
    }
}

pub fn save_poll(guild_id: GuildId, user_id: &UserId, poll: &Poll, votes: Vec<Vote>) {
    let database = get_database();
    let poll_model = PollModel::from(poll, votes, user_id);
